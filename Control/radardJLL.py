#!/usr/bin/env python3
"""  JLL  2023.7.11
from LP091/selfdrive/controls/radard.py
for 230708 KF 2
(sconsvenv) jinn@Liu:~/openpilot$ python radardJLL.py
"""
import importlib
import math
...
def radard_thread(sm=None, pm=None, can_sock=None):
  ...
  for i in range(1):
    ...
    can_strings = messaging.drain_sock_raw(can_sock, wait_for_one=False)
      #--- can_strings = []
    rr = RI.update(can_strings)
      #--- rr = (errors = [canError], points = [])
    ...
    dat = RD.update(sm, rr)
      #--- dat = ( logMonoTime = 1737251127976039,
      #---        radarState = (angleOffsetDEPRECATED = 0,calStatusDEPRECATED = 0,cumLagMs = 0,mdMonoTime = 0,
      #---          ftMonoTimeDEPRECATED = 0,calCycleDEPRECATED = 0,calPercDEPRECATED = 0,carStateMonoTime = 0,
      #---          radarErrors = [canError] ),
      #---        valid = false )
    ...
    tracks = RD.tracks
      #--- tracks = defaultdict(<class 'dict'>, {})
      #print('#--- vars(tracks) =', vars(tracks))
