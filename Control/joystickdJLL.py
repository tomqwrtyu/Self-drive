...
''' JLL  2023.3.5, 4.13
for 230305, 230412
from /openpilot/selfdrive/tools/joystick/joystickd.py
'''
import os
import argparse
import threading
...
def send_thread(joystick):
  ...
  pm = messaging.PubMaster(['testJoystick'])  # JLL
  rk = Ratekeeper(100, print_delay_threshold=None)
  while 1:
    dat = messaging.new_message('testJoystick')
    dat.testJoystick.axes = [joystick.axes_values[a] for a in joystick.axes_order]
    dat.testJoystick.buttons = [joystick.cancel]
    #joystick_sock.send(dat.to_bytes())  # JLL
    pm.send('testJoystick', dat)  # JLL
