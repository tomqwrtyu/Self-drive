...
see JLL below for Changes to OP091 version
...
State = log.ControlsState.OpenpilotState
  # enum OpenpilotState {disabled @0; preEnabled @1; enabled @2; softDisabling @3; overriding @4;}
...
ACTIVE_STATES = (State.enabled, State.softDisabling, State.overriding)  # (2, 3, 4)
  #--- State.enabled = 2
...
class Controls:
  def __init__(self, sm=None, pm=None, can_sock=None, CI=None):
    config_realtime_process(4, Priority.CTRL_HIGH)  # 4 cores
    ...
    if can_sock is None:
      can_timeout = None if os.environ.get('NO_CAN_TIMEOUT', False) else 20
        # get('NO_CAN_TIMEOUT', False) returns False if 'NO_CAN_TIMEOUT' does not exist
    ...
    CarInterface, CarController, CarState = interfaces["COMMA BODY"]  # NG KeyError: 'body'
    self.CP = CarInterface.get_non_essential_params("COMMA BODY")
      #--- self.CP = ( carName = "body", carFingerprint = "COMMA BODY", enableGasInterceptor = false,
      #--- np.size(self.CP) = 1, np.size(np.array(self.CP)) = 1
    self.CI = CarInterface(self.CP, CarController, CarState)
      #--- vars(self.CI) = {
        #--- 'CP': <car.capnp:CarParams builder (carName = "body", ...)>,
        #--- 'VM': <...>, 'frame': 0, 'steering_unpressed': 0, 'low_speed_alert': False, 'no_steer_warning': False, 'silent_steer_warning': True, 'v_ego_cluster_seen': False,
        #--- 'CS': <...>, 'can_parsers': [<...>, , , , ], 'cp': <...>, 'cp_cam': None, 'cp_adas': None, 'cp_body': None, 'cp_loopback': None,
        #--- 'CC': <...> }
    CI = self.CI
  ...
  def set_initial_state(self):  # Initialize ControlsState for replay only
    if REPLAY:
  ...
  def update_events(self, CS):  # Compute carEvents from carState
  ...
  def data_sample(self):  # Receive data from sockets and update carState
  ...
  def state_transition(self, CS):  # Compute conditional state transitions and execute actions on state transitions
  ...
  def state_control(self, CS):  # Return a CarControl packet, given the state
      # Update VehicleModel
    ...
    actuators.longControlState = self.LoC.long_control_state
      #--- actuators.longControlState = off
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):  # Send actuators and hud commands to the car, send controlsstate and MPC logging
    ...
    if (self.sm.frame % int(1. / DT_CTRL) == 0) or (self.events.names != self.events_prev):
      ...
      self.pm.send('carEvents', ce_send)  # who sm 'carEvents'?
  ...
  def step(self):
    ...
    self.prof.checkpoint("State transition")
      ...
      #--- enum EventName { joystickDebug @34; ... carUnrecognized @66; controlsInitializing @98; ...}
  ...
  def controlsd_thread(self):
    #while True:
      ...
      self.prof.display()
        #--- vars(self.sm) = {'frame': 0, 'updated': {'deviceState': False, 'pandaStates': False, 'peripheralStat
        #print('#--- self.CP =', self.CP)  # see 230714
        #print('#--- self.CS_prev =', self.CS_prev)  # see 230714
      print('#--- self.CC =', self.CC)  # see 230714
