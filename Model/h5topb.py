'''    YPL, JLL, YJW, 2021.9.17 - 2024.1.21
--- Step 1: from h5 to pb
    (sconsvenv) jinn@Liu:~/openpilot/aJLL/Model$ python h5topb.py B6.pb
...
--- Step 2: move /home/jinn/openpilot/aJLL/Model/saved_model/B6.pb to
                 /home/jinn/snpe/dlc/B6.pb
...
--- Step 3: from pb to dlc
    ...
    (snpe) jinn@Liu:~/snpe$ snpe-tensorflow-to-dlc --input_network ./dlc/B6.pb \
--input_dim Input "1,12,128,256" --input_dim Input_1 "1,8" --input_dim Input_2 "1,2" --input_dim Input_3 "1,512" \
--out_node "Identity" --output_path ./dlc/B6.dlc

--- Step 4: from dlc to html
    (snpe) jinn@Liu:~/snpe/dlc$ snpe-dlc-viewer -i B6.dlc
'''
import os
import argparse
...
def h5_to_pb(pb_save_name):
  ...
  full_model = full_model.get_concrete_function(
    [tf.TensorSpec(model.inputs[i].shape, model.inputs[i].dtype) for i in range(len(model.inputs))] )
  ...
  tf.io.write_graph(graph_or_graph_def=frozen_func.graph, logdir="./saved_model",
                    name=pb_save_name, as_text=False)
