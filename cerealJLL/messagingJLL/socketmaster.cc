...
#include "cerealJLL/servicesJLL.h"
#include "cerealJLL/messagingJLL/messaging.h"
...
void SubMaster::update(int timeout) {
  ...
  std::vector<std::pair<std::string, cerealJLL::Event::Reader>> messages;
  ...
  for (auto s : sockets) {
    ...
    delete msg;
    messages.push_back({m->name, m->msg_reader->getRoot<cerealJLL::Event>()});
  }

  update_msgs(current_time, messages);
}

void SubMaster::update_msgs(uint64_t current_time, const std::vector<std::pair<std::string, cerealJLL::Event::Reader>> &messages){
...
cerealJLL::Event::Reader &SubMaster::operator[](const char *name) const {
